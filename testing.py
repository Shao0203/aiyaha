...
class TwoLayerNet:
    ...
    def save_model(self, filepath):
        ...
        print(f'Save model into file {filepath}.')

    def load_model(self, filepath):
        with open(filepath, 'rb') as f:
            self.params = pickle.load(f)
        print(f'Load model from file {filepath}.')


# 4. Compare num_grad and gradient
...
for i in range(iters_num):
    ...
    if i % iter_per_epoch == 0:
        ...
        print(
            f'epoch {int(i // iter_per_epoch)}: train, test | {train_acc:.2%}, {test_acc:.2%}')


fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
x_loss = np.arange(iters_num)
